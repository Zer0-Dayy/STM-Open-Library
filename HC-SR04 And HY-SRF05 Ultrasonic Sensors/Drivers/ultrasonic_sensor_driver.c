...
// User will declare parameters in the main.c and call this function to initialize the sensor.
...
ultrasonic_status_t Ultrasonic_Trigger(ultrasonic_Handle_t *ultrasonic_sensor)
{
    ...
    // Important note: HAL_TIM_PWM_Start() can't send an output unless the counter register is enabled, so it arms enables the
    // channel output and prepares CCR, ARR preload. Output begins only after we use the __HAL_TIM_ENABLE() to enable the count
    // register again.

    return ULTRASONIC_OK;
}

// Defining the Input Capture callback in non-blocking mode. (It triggers automatically after the input capture detects a
// rising/falling edge)
    ...
    for (uint8_t i = 0; i < sensor_count; i++)
    {
        ...
        if (htim == sensor->htim_echo)
        {
                ...
                (active_channel == HAL_TIM_ACTIVE_CHANNEL_4 && sensor->echo_channel == TIM_CHANNEL_4))
            {
                ...
                {
                    // If the input capture is set to waiting for a rising edge, that means we must switch it to falling edge to
                    // detect when the sensor's ECHO pin goes from HIGH to LOW (sound wave returned).
                    sensor->t_rise = __HAL_TIM_GET_COMPARE(sensor->htim_echo, sensor->echo_channel);

                    TIM_IC_InitTypeDef sConfig = {0}; // Declare an sConfig structure to change the attributes of the capture input channel.
                    sConfig.ICPolarity = TIM_INPUTCHANNELPOLARITY_FALLING; // Tells the timer which edge it latches the CNT register to the CCR.
                ...
                {
                    // If the input capture is waiting for a falling edge, that means we need to read the time and switch back to
                    // the rising edge polarity. But we must inform the user/board that there is information waiting to be fetched.
...
float Ultrasonic_GetDistance(ultrasonic_Handle_t *ultrasonic_sensor)
{
    ...
    // Calculate the actual distance. (Distance = Speed of sound * Difference(t_fall - t_rise) /2)
